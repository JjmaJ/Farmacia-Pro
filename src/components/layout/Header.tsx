    ...
    <header className="bg-white/90 backdrop-blur-md border-b border-slate-200/80 px-6 py-3.5 sticky top-0 z-20 transition-all duration-300">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5">
          ...
          {/* ===== BOTÓN MANTENIMIENTO (Solo Administradores) ===== */}
          {isAdmin && (
            <Tooltip
              content={isMaintenanceMode ? '⚠️ Mantenimiento ACTIVO: Clic para desactivar' : '🛠️ Activar Modo Mantenimiento'}
              position="bottom"
            >
              <button
                onClick={toggleMaintenanceMode}
                className={`hidden sm:flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold transition-all duration-200 border ${
                  isMaintenanceMode
